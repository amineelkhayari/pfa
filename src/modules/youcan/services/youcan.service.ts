...
export class YouCanService {
  ...
  async exchangeCode(c: YouCanCredentials, code: string) {
    ...
    if (!response.ok || !body.access_token)
      throw new BadGatewayException(`YouCan OAuth failed (${response.status}): ${body.message ?? response.statusText}`);
    ...
    return {
      policies: pages
        ...
        .map((page: any) => ({
          type: page.slug,
          title: page.name,
          content: String(page.content ?? '').slice(0, 4000),
          url: page.public_url,
        })),
      shipping: shipping
        .slice(0, 20)
        .map((zone: any) => ({
          ...
          rates: (zone.rates?.data ?? zone.rates ?? []).slice(0, 20),
        })),
      payments: payments
        .slice(0, 20)
        .map((payment: any) => ({
          ...
          active: payment.active ?? payment.enabled ?? payment.is_active,
          description: String(payment.description ?? '').slice(0, 1000),
        })),
    };
  }
  async validate(c: YouCanCredentials) {
    this.validateConfig(c);
    if (c.accessToken) await this.getStore(c);
  }

  async sync(c: YouCanCredentials, storeId: string) {
    const [products, orders] = await Promise.all([
      this.all(c, '/products?include=variants,images'),
      this.all(c, '/orders?include=customer,variants,payment,shipping'),
    ]);
    const productRows = products.map((p: any) => this.mapProduct(p, storeId));
    if (productRows.length) await this.products.upsert(productRows as any, ['storeId', 'externalProductId']);
    const orderRows = orders.map((o: any) => this.mapOrder(o, storeId));
    if (orderRows.length) await this.orders.upsert(orderRows as any, ['storeId', 'externalOrderId']);
  ...
  async importOrder(payload: any, storeId: string) {
    ...
    await this.orders.upsert(row as any, ['storeId', 'externalOrderId']);
    const saved = await this.orders.findOneBy({ storeId, externalOrderId: String(source.id) });
  ...
  findOrder(storeId: string, id: string) {
    return this.orders.findOneBy({ storeId, externalOrderId: id });
  }
  ...
  async ensureWebhooks(c: YouCanCredentials, storeId: string) {
    ...
    for (const event of ['order.created', 'order.updated', 'order.paid', 'app.uninstalled']) {
      ...
      try {
        await this.request(c, '/resthooks/subscribe', {
          method: 'POST',
  ...
  verifyWebhook(raw: Buffer, signature: string | undefined, secret: string) {
    ...
    return [expectedHex, expectedBase64].some(value => {
  ...
  async updateOrderStatus(c: YouCanCredentials, id: string, status: string) {
    return this.request(c, `/orders/${id}/status`, { method: 'PUT', body: JSON.stringify({ status }) });
  }
  async confirmOrder(c: YouCanCredentials, id: string) {
    return this.updateOrderStatus(c, id, 'processed');
  }
  async cancelOrder(c: YouCanCredentials, id: string) {
    return this.updateOrderStatus(c, id, 'canceled-by-seller');
  }
  async updateOrderShippingAddress(c: YouCanCredentials, id: string, address: Record<string, unknown>) {
    return this.request(c, `/orders/${id}`, { method: 'PUT', body: JSON.stringify({ shipping: address }) });
  }
  ...
  async createConfirmedChatOrder(
    ...
    input: {
      ...
      country: string;
      shippingEstimationId: string;
    },
  ) {
    if (!input.shippingEstimationId)
      throw new BadRequestException(
        'Configure a default YouCan shipping estimation ID before creating WhatsApp orders.',
      );
    const [firstName, ...lastName] = input.customerName.trim().split(/\s+/);
    const customer = await this.request(c, '/customers', {
      method: 'POST',
      body: JSON.stringify({
    ...
    const address = {
      ...
      last_name: lastName.join(' '),
      phone: input.phone,
      first_line: input.address1,
      city: input.city,
      zip_code: input.postalCode ?? '',
      country_code: input.country,
    };
    const order = await this.request(c, '/orders', {
      method: 'POST',
      body: JSON.stringify({
        customer_id: customer.id,
        variants: [{ quantity: input.quantity, price: input.price, variant: { id: input.variantId } }],
  ...
  private validateConfig(c: YouCanCredentials) {
    if (!c.clientId || !c.clientSecret || !c.redirectUri)
      throw new BadRequestException('YouCan Client ID, Client Secret, and Redirect URI are required.');
  }
  ...
  private mapProduct(p: any, storeId: string) {
    ...
    return {
      ...
      handle: p.slug ?? null,
      productType: p.type ?? p.category?.name ?? null,
      vendor: p.vendor ?? null,
      status: p.status ?? (p.visibility ? 'active' : 'draft'),
      tags: p.tags ?? null,
      imageUrl: images[0]?.url ?? images[0]?.src ?? p.thumbnail ?? null,
      variants,
      price: Number(p.price ?? variants[0]?.price ?? 0),
      externalCreatedAt: p.created_at ? new Date(p.created_at) : new Date(),
      externalUpdatedAt: p.updated_at ? new Date(p.updated_at) : new Date(),
    };
  }
  private mapOrder(o: any, storeId: string) {
    const customer = o.customer?.data ?? o.customer ?? {};
    const shipping = o.shipping?.data ?? o.shipping ?? o.shipping_address ?? {};
    const payment = o.payment?.data ?? o.payment ?? {};
    const items = o.variants?.data ?? o.variants ?? o.items ?? [];
    const status = String(o.status?.slug ?? o.status_new ?? o.status_text ?? o.status ?? 'open').toLowerCase();
    const financialStatus = String(
      payment.status_text ?? o.payment_status ?? (o.paid_at ? 'paid' : 'pending'),
    ).toLowerCase();
    const fulfillmentStatus = String(shipping.status_text ?? o.shipping_status ?? status).toLowerCase();
    const providerTags = (Array.isArray(o.tags?.data) ? o.tags.data : Array.isArray(o.tags) ? o.tags : [])
      .map((tag: any) => String(tag?.name ?? tag?.slug ?? tag).trim())
      .filter(Boolean);
    const createdByWhatsApp = providerTags.some((tag: string) =>
      ['whatsapp-confirmed', 'whatsapp-bot-confirmed'].includes(tag.toLowerCase()),
    );
    return {
      storeId,
      externalOrderId: String(o.id),
      orderNumber: String(o.ref ?? o.number ?? o.id),
      email: customer.email ?? o.email ?? null,
      phone: customer.phone ?? shipping.address?.phone ?? shipping.phone ?? o.phone ?? null,
      customerName:
        customer.full_name ??
        customer.name ??
        ([customer.first_name, customer.last_name].filter(Boolean).join(' ') || null),
      totalPrice: Number(o.total ?? o.total_price ?? 0),
      currency: o.currency?.code ?? o.currency ?? 'MAD',
      financialStatus,
      fulfillmentStatus,
      lineItems: items,
