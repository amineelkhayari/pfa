...
export function validateEnv(config: EnvConfig): EnvConfig {
  ...
  if (dbType === 'postgres') {
    ...
    // SQLite (explicit or default): DATABASE_NAME is a file path.
    ...
    // PostgreSQL DATABASE_NAME (e.g. 'smartConfirm') leaking into a SQLite run (#677). That bare name becomes
    // the SQLite file PATH, opening a file named 'smartConfirm' under the read-only app rootfs →
    ...
    if (dataDbName && !dataDbName.includes('/') && !dataDbName.includes('\\') && !/\.(sqlite|db)$/i.test(dataDbName)) {
      errors.push(
        `DATABASE_NAME must be a file path under the data volume for SQLite (e.g. ./data/smartConfirm.sqlite); got ${JSON.stringify(
          dataDbName,
        )}. A bare name is the PostgreSQL DB name — leave DATABASE_NAME unset for SQLite to use the default ./data/smartConfirm.sqlite.`,
      );
    }
  }
