...
describe('configuration — Postgres database name', () => {
  ...
  it('resolves dataDatabase.name from DATABASE_NAME (matches the migration CLI), default smartConfirm', () => {
    delete process.env.DATABASE_NAME;
    expect(configuration().dataDatabase.name).toBe('smartConfirm');
...
describe('configuration — plugins directory default', () => {
  ...
  it('lets an explicit PLUGINS_DIR win, and drops the legacy fallback with it', () => {
    process.env.PLUGINS_DIR = '/srv/smartConfirm-plugins';
    const cfg = configuration();
    expect(cfg.plugins.dir).toBe('/srv/smartConfirm-plugins');
