...
export function RoleProvider({ children }: { children: ReactNode }) {
  const [role, setRoleState] = useState<UserRole | null>(() => {
    const saved = localStorage.getItem('smartConfirm_user_role');
  ...
  const setRole = useCallback((newRole: UserRole | null) => {
    ...
    if (newRole) {
      localStorage.setItem('smartConfirm_user_role', newRole);
    } else {
      localStorage.removeItem('smartConfirm_user_role');
    }
  }, []);
