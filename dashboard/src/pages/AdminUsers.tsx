    ...
    <div className="admin-users-table"><div className="admin-user-row header"><span>User</span><span>Plan</span><span>Messages</span><span>Status</span><span>Details</span></div>
      {isLoading ? <p className="admin-loading">Loading users…</p> : users.map(user => <Fragment key={user.id}><div className={`admin-user-row ${selectedId === user.id ? 'selected' : ''}`}>
        ...
        <div className="user-detail-grid"><section><h4>Account</h4><dl><div><dt>Email</dt><dd>{details.user.email}</dd></div><div><dt>Username</dt><dd>@{details.user.username}</dd></div><div><dt>Role</dt><dd>{details.user.role}</dd></div><div><dt>Created</dt><dd>{new Date(details.user.createdAt).toLocaleString()}</dd></div><div><dt>Last updated</dt><dd>{new Date(details.user.updatedAt).toLocaleString()}</dd></div><div><dt>Usage cycle began</dt><dd>{new Date(details.usagePeriodStart).toLocaleString()}</dd></div></dl></section>
          ...
          <section><h4>Billing subscriptions</h4>{details.subscriptions.length ? details.subscriptions.map(subscription => { const schedule = subscriptionSchedule(subscription); return <div className="subscription-line" key={subscription.id}><div><strong>{subscription.provider.toUpperCase()} · {subscription.planSlug}</strong><small>{subscription.status} · {schedule.autoRenew ? 'auto-renew on' : 'auto-renew off'}</small>{subscription.pendingPlanSlug && <small>Change to {subscription.pendingPlanSlug}: {subscription.planChangeStatus.replaceAll('_', ' ')}</small>}{subscription.planChangeError && <small className="billing-error">{subscription.planChangeError}</small>}</div><div className="subscription-renewal"><small>{subscription.pendingPlanSlug ? 'Change effective' : schedule.label}</small><strong>{formatBillingDate(subscription.pendingPlanSlug ? subscription.planChangeEffectiveAt : schedule.date)}</strong></div></div>; }) : <p>No payment subscription. The plan may be free or assigned manually.</p>}</section>
        </div></>}</div>}</Fragment>)}
    </div>
  </div>;
}
