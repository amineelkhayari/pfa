...
async function main() {
  ...
  const result = await client.messages.sendText('my-session', {
    ...
    text: 'Hello from the SmartConfirm SDK!',
  });
...
The constructor takes a single `SmartConfirmClientOptions` object. `baseUrl` and `apiKey` are required (the constructor throws synchronously if either is missing).

| Option | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `baseUrl` | `string` | yes | — | Base URL of the SmartConfirm API, e.g. `http://localhost:2785`. A trailing slash is trimmed; a path prefix (e.g. `https://host/v1`) is preserved. |
...
All resources are accessed as properties on the client (`client.<resource>.<method>`). Every method returns a `Promise`. Methods marked **OPERATOR** require an `OPERATOR`-level API key (an `ADMIN` key satisfies it); a `VIEWER` key receives a `403` (`SmartConfirmForbiddenError`).
...
On a non-2xx response the SDK throws a typed `SmartConfirmApiError` subclass carrying `.status` (HTTP status), `.body` (parsed JSON error envelope, or raw text), and `.errorKind` (the NestJS `error` field). All error classes extend `SmartConfirmError` and are exported, so they are `instanceof`-checkable. A timeout throws `SmartConfirmTimeoutError`, which extends `SmartConfirmError` directly (not `SmartConfirmApiError`).

| Error class | HTTP status | Meaning |
| --- | --- | --- |
| `SmartConfirmAuthError` | 401 | Missing or invalid API key. |
| `SmartConfirmForbiddenError` | 403 | The key's role is insufficient (e.g. an OPERATOR-only route). |
| `SmartConfirmNotFoundError` | 404 | Resource not found. |
| `SmartConfirmConflictError` | 409 | Conflict — typically the engine is not ready. |
| `SmartConfirmRateLimitError` | 429 | Rate limited. |
| `SmartConfirmNotImplementedError` | 501 | The active engine does not support this operation. |
| `SmartConfirmApiError` | any other | Generic non-2xx (the base API error, e.g. `400`; also surfaced for unfollowed 3xx redirects). |
| `SmartConfirmTimeoutError` | — | The request exceeded the configured timeout. |
...
try {
  ...
  if (err instanceof SmartConfirmConflictError) {
    // 409 — engine not ready; start the session first.
  } else if (err instanceof SmartConfirmNotFoundError) {
    // 404 — session/chat does not exist.
  } else if (err instanceof SmartConfirmRateLimitError) {
    // 429 — back off and retry.
  } else if (err instanceof SmartConfirmTimeoutError) {
    // request timed out.
  } else if (err instanceof SmartConfirmApiError) {
...
- **Redirects are never followed.** The transport uses `redirect: 'manual'`, so a `3xx` surfaces to the caller as an error (via `SmartConfirmApiError`) rather than being followed — this guarantees the `X-API-Key` header is never re-sent to a redirect target (potentially a different origin).
...
result = client.messages.send_text("my-session", {
    "chatId": "628123456789@c.us",
    "text": "Hello from the SmartConfirm Python SDK!",
})
...
`SmartConfirmClient` is also a context manager, so the connection pool is closed for you:

```python
with SmartConfirmClient(base_url="http://localhost:2785", api_key="owa_k1_…") as client:
...
| `timeout` | `float` | `30.0` | Per-request timeout in seconds. A breach raises `SmartConfirmTimeoutError`. |
...
Every error inherits from `SmartConfirmError`. A non-2xx response raises an `SmartConfirmApiError` (or a more specific subclass picked by status); a timeout raises `SmartConfirmTimeoutError`. The API-error classes carry `.status` (HTTP code), `.body` (parsed JSON or raw text), and `.error_kind` (the NestJS `error` field).

| Exception | Trigger |
| --- | --- |
| `SmartConfirmAuthError` | HTTP `401` — missing or invalid API key |
| `SmartConfirmForbiddenError` | HTTP `403` — insufficient role |
| `SmartConfirmNotFoundError` | HTTP `404` — resource not found |
| `SmartConfirmConflictError` | HTTP `409` — typically engine-not-ready |
| `SmartConfirmRateLimitError` | HTTP `429` — too many requests |
| `SmartConfirmNotImplementedError` | HTTP `501` — active engine doesn't support the operation |
| `SmartConfirmApiError` | any other non-2xx status (incl. unfollowed `3xx`) |
...
client = SmartConfirmClient(base_url="http://localhost:2785", api_key="owa_k1_…")
...
except SmartConfirmConflictError:
    print("Session engine not ready yet — start it first.")
except SmartConfirmNotFoundError:
    print("Session does not exist.")
except SmartConfirmRateLimitError:
    print("Rate limited — back off and retry.")
except SmartConfirmTimeoutError as e:
    print(f"Timed out after {e.timeout}s")
except SmartConfirmApiError as e:
    print(f"API error {e.status} ({e.error_kind}): {e.body}")
```

### Notable Behaviors

- **Redirects are never followed.** `follow_redirects` is forced off so the `X-API-Key` header is never re-sent to a redirect target. An unfollowed `3xx` therefore surfaces as an `SmartConfirmApiError` rather than a success.
...
The PHP SDK is a hand-written, synchronous client built on Guzzle 7. It mirrors the full user-facing API surface: every resource method maps to one REST endpoint, request/response payloads are plain associative arrays, and non-2xx responses are translated into a typed `SmartConfirm*Exception` hierarchy.
...
- PSR-4 autoloaded under the `SmartConfirm\` namespace (`"SmartConfirm\\": "src/"`).
...
Missing `baseUrl` or `apiKey` throws `SmartConfirm\Exceptions\SmartConfirmException` from the constructor.
...
All exceptions live in `SmartConfirm\Exceptions` and descend from `SmartConfirmException` (which extends PHP's `\Exception`). Any non-2xx response is raised as an `SmartConfirmApiException`; the static `classify()` factory picks the most specific subclass by status code. An `SmartConfirmApiException` carries the HTTP status (`getStatus(): int`), the parsed error body (`getBody(): mixed`), and the NestJS `error` kind when present (`getErrorKind(): ?string`).

| Exception | Extends | Trigger |
| --- | --- | --- |
| `SmartConfirmException` | `\Exception` | Base for all SDK errors (also thrown for missing `baseUrl`/`apiKey`). |
| `SmartConfirmApiException` | `SmartConfirmException` | Any non-2xx (including unfollowed 3xx and other 4xx/5xx). |
| `SmartConfirmAuthException` | `SmartConfirmApiException` | `401` — missing/invalid API key. |
| `SmartConfirmForbiddenException` | `SmartConfirmApiException` | `403` — insufficient role (e.g. operator-only endpoint). |
| `SmartConfirmNotFoundException` | `SmartConfirmApiException` | `404` — resource not found. |
| `SmartConfirmConflictException` | `SmartConfirmApiException` | `409` — conflict (e.g. engine not ready). |
| `SmartConfirmRateLimitException` | `SmartConfirmApiException` | `429` — rate limited. |
| `SmartConfirmNotImplementedException` | `SmartConfirmApiException` | `501` — active engine does not support the operation. |
| `SmartConfirmTimeoutException` | `SmartConfirmException` | Request exceeded the timeout (`getTimeout(): float`). Not an API error — has no status/body. |

```php
<?php
use SmartConfirm\Client;
use SmartConfirm\Exceptions\SmartConfirmConflictException;
use SmartConfirm\Exceptions\SmartConfirmRateLimitException;
use SmartConfirm\Exceptions\SmartConfirmTimeoutException;
use SmartConfirm\Exceptions\SmartConfirmApiException;
...
} catch (SmartConfirmConflictException $e) {
    // 409 — engine not ready yet
} catch (SmartConfirmRateLimitException $e) {
    // 429 — back off and retry yourself (no auto-retry)
} catch (SmartConfirmTimeoutException $e) {
    fwrite(STDERR, "timed out after {$e->getTimeout()}s\n");
} catch (SmartConfirmApiException $e) {
    // any other non-2xx
...
- **Redirects are never followed.** Guzzle is configured with `allow_redirects => false`, so a `3xx` surfaces as an `SmartConfirmApiException` rather than being followed — the `X-API-Key` header is never re-sent to a redirect target.
...
SmartConfirm's n8n integration is **not** part of these SDK packages. It is a separate community node maintained in its own repository, which speaks the same REST + webhook contract documented in [API Specification](./06-api-specification.md):

- An **action/HTTP** path that calls the REST endpoints (e.g. `POST /api/sessions/:id/messages/send-text`) with the `X-API-Key` header.
- A **trigger** path that registers a webhook (the `webhooks` resource) and receives inbound events, verifying the `X-SmartConfirm-Signature` HMAC.
