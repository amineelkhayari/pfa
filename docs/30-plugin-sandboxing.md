...
SmartConfirm runs **untrusted plugins** (anything loaded from the plugins directory) in an isolated worker
...
as the same uid as SmartConfirm. The sandbox therefore does **not**, by itself, stop a malicious plugin
from reading files the SmartConfirm process can read or making outbound network connections. It protects
