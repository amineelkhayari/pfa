...
The overriding goal is to preserve the untrusted-worker safety invariants *by construction*. SmartConfirm
...
**~90% a faithful clone of seams SmartConfirm already ships**:
...
flowchart LR
    ...
    subgraph Core["SmartConfirm host (core)"]
...
- **Authentication inversion.** A provider webhook cannot carry an SmartConfirm API key, so ingress is public to
...
[SmartConfirm-plugins](https://github.com/your-organization/smartconfirm-plugins) catalog, not in this repository. What
