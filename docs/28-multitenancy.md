...
> architecture for operating SmartConfirm as a multi-tenant platform: tenant identity, access control, 2FA,
...
- One SmartConfirm deployment safely hosts many independent organizations (tenants), each with its own
...
SmartConfirm today is **multi-session, not multi-tenant**. What already exists and is reused by this design:
...
- **Linked-device identity**: the engine's browser display name (the `BAILEYS_BROWSER` tuple)
  ...
  `BAILEYS_BROWSER_NAME` env → `'SmartConfirm'`. This supersedes the global-only env approach for
