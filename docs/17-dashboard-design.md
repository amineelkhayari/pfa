...
The dashboard is a web-based management interface for SmartConfirm that lets users manage sessions, webhooks, and monitor activity without using the API directly.
...
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │  🔗 Main Webhook                                   ✅ Active   │  │
│  │  https://api.example.com/webhook/smartConfirm                        │  │
...
async function request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  const apiKey = sessionStorage.getItem('smartConfirm_api_key');
...
`localStorage` under `smartConfirm_theme`:
...
maintainability; the legacy `smartConfirm_palette` storage key and the attribute are cleaned up on load.
...
export function useTheme() {
  const [theme, setTheme] = useState<Theme>(/* localStorage 'smartConfirm_theme' ?? 'system' */);

  useEffect(() => {
    const root = document.documentElement;
    if (theme === 'system') root.removeAttribute('data-theme');
    else root.setAttribute('data-theme', theme);
    localStorage.setItem('smartConfirm_theme', theme);
  }, [theme]);
