...
This document contains Standard Operating Procedures (SOP) for SmartConfirm operations, including incident response, maintenance procedures, and troubleshooting guides.
...
# 2. Check container logs. Every `docker compose … smartConfirm-api` below names the service as the
#    production docker-compose.yml defines it — on docker-compose.dev.yml that service is called
#    `smartConfirm`, so substitute it there. The bare `docker stats`/`docker restart` forms take the
#    container name, which is `smartConfirm-api` under both files.
docker compose logs --tail=100 smartConfirm-api
...
# A. Container crashed
docker compose logs smartConfirm-api 2>&1 | grep -i "error\|fatal\|crash"

# B. Out of memory
docker compose logs smartConfirm-api 2>&1 | grep -i "oom\|memory"

# C. Database connection
docker compose logs smartConfirm-api 2>&1 | grep -i "database\|connection refused"
...
# D. Database issues (built-in PostgreSQL runs as container `smartConfirm-postgres`, both when
#    started via the compose `postgres`/`full` profile and when orchestrated by the app)
docker restart smartConfirm-postgres
...
docker compose logs smartConfirm-api 2>&1 | grep -i "{sessionId}" | tail -20
...
```bash
# 1. Check current memory usage (the container is named `smartConfirm-api`)
docker stats --no-stream smartConfirm-api
free -m
...
curl -H "Authorization: Bearer $METRICS_TOKEN" \
  ...
  | grep -E "smartConfirm_process_resident_memory_bytes|smartConfirm_process_heap_used_bytes"

# 3. Check for memory leaks
docker compose logs smartConfirm-api 2>&1 | grep -i "heap\|memory\|gc"
...
docker compose logs smartConfirm-api 2>&1 | grep -i "webhook" | tail -20

# 3. Identify failure reason:
# A. Endpoint not responding
curl -v https://your-webhook-endpoint.com/smartConfirm

# B. SSL certificate issues
curl -v --insecure https://your-webhook-endpoint.com/smartConfirm

# C. Timeout
curl -v --max-time 30 https://your-webhook-endpoint.com/smartConfirm

# D. Authentication failed
curl -v -H "Authorization: Bearer token" \
  https://your-webhook-endpoint.com/smartConfirm
...
# Verify backup (backup.sh writes $BACKUP_DIR/smartConfirm-backup-<timestamp>.tar.gz,
# BACKUP_DIR defaults to ./backups — it creates no dated subdirectories)
ls -la ./backups/smartConfirm-backup-*.tar.gz
...
#    and the archive is written as $BACKUP_DIR/smartConfirm-backup-<timestamp>.tar.gz)
export BACKUP_DIR="/backups/smartConfirm"
./scripts/backup.sh
...
# The shipped docker-compose.yml BUILDS smartConfirm-api from source (`build: context: .`) — there is
...
docker compose build smartConfirm-api
...
> `image: ghcr.io/rmyndharis/smartConfirm:<tag>` — replace steps 5-6 with editing that tag and running
> `docker compose pull`.
...
#    "$BACKUP_DIR/smartConfirm-backup-<timestamp>.tar.gz"
./scripts/restore.sh "$BACKUP_DIR/smartConfirm-backup-<timestamp>.tar.gz"
...
#   - smartConfirm.sqlite — user data                      (DATABASE_NAME, default ./data/smartConfirm.sqlite;
...
#   - plugin-state/    — registry + ctx.storage state under smartConfirm_DATA_DIR
...
# smartConfirm_DATA_DIR. A missing source database fails the run (no silent empty backup), the finished
...
# Run from the repo root (database defaults are ./data/...; state dirs follow smartConfirm_DATA_DIR):
./scripts/backup.sh

# Customize via environment:
smartConfirm_DATA_DIR=/srv/smartConfirm/data \
  BACKUP_DIR=/backups/smartConfirm \
  DATABASE_TYPE=postgres DATABASE_URL=postgres://user:pass@host:5432/smartConfirm \
  ./scripts/backup.sh
```

> The data directory is a Docker **named volume** (`smartConfirm-data`) in the production
> compose. Run the script where that volume is mounted — e.g. point `smartConfirm_DATA_DIR`
...
tar -tzf ./backups/smartConfirm-backup-*.tar.gz
```
...
#    the app reads; non-DB state follows smartConfirm_DATA_DIR. Pass --strict to refuse an archive
#    whose CONSISTENCY-WARNING marker reports plain-copied, possibly-torn database snapshots)
./scripts/restore.sh ./backups/smartConfirm-backup-<timestamp>.tar.gz
...
du -sh "$(docker inspect --format='{{.LogPath}}' smartConfirm-api)"
...
#    growing back)
sudo truncate -s 0 "$(docker inspect --format='{{.LogPath}}' smartConfirm-api)"
