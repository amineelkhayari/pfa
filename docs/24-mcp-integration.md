...
>
> SmartConfirm can expose a curated set of its capabilities to AI agents over the
...
as first-class tools. SmartConfirm's MCP integration exposes a **focused, curated** slice of
...
2. **No vendor lock-in.** Core never imports the MCP SDK. The `@modelcontextprotocol/sdk`
   ...
   is enabled.
3. **Curated surface.** Rather than reflecting every REST route, SmartConfirm exposes an
