...
This example shows how to use SmartConfirm and n8n to collect appointment requests over WhatsApp, check availability in an external system, and send a confirmation or alternative time slots.
...
[Availability Source]
      │
      ├── available → [Create Booking] → [SmartConfirm: Send Text confirmation]
      │
      └── unavailable → [SmartConfirm: Send Text with alternative slots]
```

## Trigger

Use the **SmartConfirm Trigger** node.

| Field | Value |
| ----- | ----- |
| Event | `message.received` |
| Session | Your connected SmartConfirm session |
...
When the requested slot is available, create the booking in your scheduling source and send a confirmation with the **SmartConfirm: Send Text** node.
...
- SmartConfirm Trigger: receives `message.received` events.
...
- SmartConfirm Send Text: sends confirmation, alternatives, or clarification.
