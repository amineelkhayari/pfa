...
# smartConfirm_env_file_value <file> <key> — print the value from one env-file layer, or nothing.
smartConfirm_env_file_value() {
...
# smartConfirm_resolve <key> <default> — the application's precedence: environment, then ./.env, then
# <data dir>/.env.generated, then the built-in default. Requires DATA_DIR to be set by the caller.
smartConfirm_resolve() {
  ...
  for layer in "./.env" "${DATA_DIR:-./data}/.env.generated"; do
    value="$(smartConfirm_env_file_value "$layer" "$key")"
    if [ -n "$value" ]; then
      printf '%s' "$value"
      return 0
    fi
